...
//   3) Run both                                      -> both
//
// Output version: point files, final hull files, and optional step snapshots.
// ============================================================

const double EPS = std::numeric_limits<double>::epsilon() * 128.0;
const double PI = acos(-1.0);
bool SAVE_STEPS = true;
...
vector<Point> generatePoints(int N, unsigned seed, const string& distribution) {
    ...
    if (distribution == "uniform" || distribution == "cube") {
    ...
    else if (distribution == "paraboloid") {
        for (int i = 0; i < N; i++) {
            double x = uni(rng);
            double y = uni(rng);
            double z = x * x + y * y;
        ...
        cerr << "Use: uniform, gaussian, sphere, clustered, paraboloid, ring, shell, ball\n";
        exit(1);
    }
...
HullOutput runGiftWrapping(vector<Point> pts) {
    ...
    auto snapshot = [&]() {
        ...
        for (auto& f : faces) {
            cur.push_back({f.a, f.b, f.c});
        }

        if (!out.steps.empty() && out.steps.back() == cur) return;
        out.steps.push_back(cur);
    };
...
private:
    ...
    void snapshot() {
        ...
        if (!steps.empty() && steps.back() == cur) return;
        steps.push_back(cur);
    }
...
int main(int argc, char** argv) {
    ...
    if (N > 10000) {
        SAVE_STEPS = false;
        cout << "[INFO] Large input: step snapshots disabled to reduce memory usage\n";
    }

    cout << "[INFO] Output mode: point/final files enabled; snapshots depend on input size\n";
    ...
    if (algorithm == "gw" || algorithm == "both") {
        ...
        if (SAVE_FILES) {
            cout << "[GW] Saved as suffix: " << suffix << "\n";
        }
    }
    ...
    if (algorithm == "naw" || algorithm == "both") {
        ...
        if (SAVE_FILES) {
            cout << "[NAW] Saved as suffix: " << suffix << "\n";
        }
    }
